...
> **查询格式**：本项目客户端（Web WASM / Flutter FFI）在发送前用 jieba-rs 分词，
> 将 `"人工智能"` 转为 `"人工 智能"`（空格分隔）后发送。服务端直接走 LIKE/tsvector/Tantivy 索引路径。
> 第三方 OPDS 客户端不做分词 → 服务端自动降级：先尝试按空格拆词，
> 词数 ≥2 走索引路径，=1 走 `raw_text LIKE '%原文%'` 兜底。

- **GET** `/opds/catalog/search?q=人工%20智能&page=1&limit=20`
- **Accept**: `application/atom+xml`

```xml
<feed xmlns="http://www.w3.org/2005/Atom"
      xmlns:opensearch="http://a9.com/-/spec/opensearch/1.1/">
  <title>搜索：人工 智能</title>
...
- **GET** `/opds/v2/catalog/search?q=人工%20智能&page=1`
- **Accept**: `application/opds+json`
...
| 参数 | 类型 | 说明 |
|------|------|------|
| `page_num` | int | 过滤：只取指定页码的标注 |
| `type` | string | 过滤：标注类型，逗号分隔（如 `highlight,note`） |
| `color` | string | 过滤：颜色值（如 `#FFFF00`） |
| `motivation` | string | 过滤：动机（如 `highlighting`） |
| `tag` | string | 过滤：标签（如 `重要`） |
| `search` | string | 全文搜索标注内容 |
| `sort` | string | 排序：`created_at:desc` / `page_num:asc` / `modified:desc` |
| `page` | int | 分页页码（从 1 开始） |
| `limit` | int | 每页数量（默认 20，最大 100） |

- **Response** `200 OK` — W3C `AnnotationPage` 分页格式:
```json
{
  "@context": "http://www.w3.org/ns/anno.jsonld",
  "type": "AnnotationPage",
  "partOf": {
    "id": "/api/v1/documents/101/annotations",
    "totalItems": 42
  },
  "next": "/api/v1/documents/101/annotations?page=2&limit=20",
  "prev": null,
  "startIndex": 0,
  "totalItems": 42,
  "items": [ /* Annotation 数组 */ ]
...
> **分页格式**：遵循 W3C Web Annotation 的 `AnnotationCollection` + `AnnotationPage` 模型
> （`as:OrderedCollection` / `as:OrderedCollectionPage`）。
> `partOf` 指向完整集合，`next`/`prev` 用于翻页，`totalItems` 为总数。
...
- **GET** `/api/v1/documents/{doc_id}/annotations/export?format=json`
  - `format=json` — Readium Annotation Set JSON（标准格式，可导入其他阅读器）
  - `format=markdown` — Markdown 文件（人类可读，按页码组织）
  - `format=html` — HTML 文件（带样式，可直接打印）
...
```json
{
  ...
  "type": "AnnotationSet",
  "about": {
    "dc:identifier": "urn:uuid:550e8400-...",
    "dc:format": "application/pdf",
    "dc:title": "深入理解计算机系统",
    "dc:creator": ["Randal E. Bryant"],
    "dc:date": "2016-11"
  },
...
> **导出格式详见** `27-标注导出与迁移方案.md`

### 5.9 标注导入

- **POST** `/api/v1/documents/{doc_id}/annotations/import`
  - `multipart/form-data`, file (Readium Annotation Set JSON)
  - 服务端验证 JSON Schema，逐条导入标注
  - 冲突处理：按 `id` 去重，已存在的跳过

> **Web 端渲染**：GET 标注后，文本型 → CSS 绝对定位覆盖；图形型 → 读取 `stroke_data` 中的 SVG path 用 Canvas/SVG 绘制。Web 端无法创建手写/涂鸦，但可**完整展示**所有类型。详见 `30-标注渲染策略.md`。
