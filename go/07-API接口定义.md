...
**Go 实现要点**（Gin Handler）：
```go
func (h *OPDSHandler) Acquisition(c *gin.Context) {
    docID, _ := strconv.Atoi(c.Param("id"))
    doc, err := h.service.GetByID(c.Request.Context(), docID)
    if err != nil {
        c.JSON(404, gin.H{"error": "not found"})
        return
    }
    // 生成 S3 预签名 URL，有效期 1 小时
    presignedURL, _ := h.s3Service.PresignGet(doc.S3Key, 1*time.Hour)

    if c.Query("format") == "url" {
        c.JSON(200, gin.H{"url": presignedURL, "expires_at": time.Now().Add(1*time.Hour)})
        return
    }
    c.Redirect(302, presignedURL)
}
```
...
func (h *DocumentHandler) Upload(c *gin.Context) {
    ...
    doc, err := h.lifecycle.ProcessNewDocument(
        c.Request.Context(), user.UserID, file, header, title, tags,
    )
    if err != nil {
...
- **POST** `/api/v1/dictionaries` — 副作用：触发全服务热更新（GSE 原生支持，无需额外开发）
...
```go
// cmd/main/main.go — OPDS + 私有 API 路由注册

func setupRoutes(r *gin.Engine, h *handler.Handlers, mw *middleware.Middleware) {
    // ===== OPDS 主干（标准协议）=====
    opds := r.Group("/opds")
    opds.Use(mw.ContentNegotiation())
    {
        opds.GET("/catalog", h.OPDS.Navigation)        // 1.1 根目录导航
        opds.GET("/catalog/*path", h.OPDS.Acquisition)  // 1.2 书目列表
        opds.GET("/search", h.OPDS.OpenSearchDesc)      // 2.1 OpenSearch
        opds.GET("/catalog/search", h.OPDS.Search)      // 2.2 搜索 (Atom)
        opds.GET("/acquisition/:id", h.OPDS.Download)   // 3.1 PDF 下载
    }

    // OPDS 2.0
    opdsV2 := r.Group("/opds/v2")
    opdsV2.Use(mw.ContentNegotiation())
    {
        opdsV2.GET("/catalog", h.OPDS.CatalogV2)             // 1.3 JSON-LD 目录
        opdsV2.GET("/catalog/search", h.OPDS.SearchV2)       // 2.3 搜索 (JSON-LD)
    }

    // ===== 私有扩展 API（Go + Gin Handler）=====
    v1 := r.Group("/api/v1")
    ...
    authenticated.Use(mw.AuthRequired())
    {
        // 用户
        authenticated.GET("/auth/me", h.Auth.Me)

        // 文档
        authenticated.POST("/documents/upload", h.Document.Upload)
        authenticated.GET("/documents/:id/status", h.Document.Status)
        authenticated.GET("/documents/:id/cover", h.Document.Cover)

        // 进度同步（OPDS Progression）
        authenticated.GET("/progression", h.Progression.ListAll)           // 增量
        authenticated.GET("/progression/:id", h.Progression.Get)
        authenticated.PUT("/progression/:id", h.Progression.Update)

        // 高亮/注释/书签（W3C Web Annotation）
        authenticated.GET("/documents/:id/annotations", h.Annotation.List)
        authenticated.POST("/documents/:id/annotations", h.Annotation.Create)
        authenticated.PUT("/annotations/:id", h.Annotation.Update)
        authenticated.DELETE("/annotations/:id", h.Annotation.Delete)

        // 词典（管理员）
        admin := authenticated.Group("/admin")
        admin.Use(mw.AdminRequired())
        {
            admin.GET("/segmenter", h.Admin.GetSegmenter)
            admin.PUT("/segmenter", h.Admin.UpdateSegmenter)
            admin.GET("/search-engine", h.Admin.GetSearchEngine)
            admin.PUT("/search-engine", h.Admin.UpdateSearchEngine)
            admin.POST("/segmenter/test", h.Admin.TestSegmenter)
            admin.GET("/metrics", h.Admin.Metrics)
        }

        // 词典
        authenticated.GET("/dictionaries", h.Dictionary.List)
        authenticated.POST("/dictionaries", h.Dictionary.Create)
        authenticated.POST("/dictionaries/import", h.Dictionary.Import)
        authenticated.GET("/dictionaries/export", h.Dictionary.Export)
        authenticated.DELETE("/dictionaries/:id", h.Dictionary.Delete)
    }
